...
{
    public class BulkCreateGroupMemberships(IHttpClientFactory httpClientFactory)
    {
        ...
        IEnumerable<User> MapCsvToListOfUsers(string csvPath, int skip)
        {
        ...
        string ConstructApiUrl(IEnumerable<User> users)
        {
        ...
        {
            HttpResponseMessage response = await httpClientFactory.CreateClient("Zendesk").GetAsync(apiUrl);
